...
class ChampData < ApplicationRecord
  ...
  # Only Champs::CarteChamp uses this one, but it is declared here so that it can
  # be included in queries. Deliberately kept apart from piece_justificative_file:
  # this image is generated by us, not supplied by the usager, and everything that
  # walks the attachments of a champ — pj exports, gallery, watermarking,
  # auto-purge — walks piece_justificative_file. Sharing that name would leave
  ...
  def clear
    ...
    ChampData.no_touching do
      ...
      # only triggered on submission, so nothing else would clean it up.
      static_map.purge_later
    end
  end
