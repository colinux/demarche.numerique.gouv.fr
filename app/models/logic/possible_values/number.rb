...
class Logic::PossibleValues::Number < Data.define(:integer, :intervals, :limits)
  ...
  # NumberLimitValidator): non-negative when `positive_number`, within the
  # validator's range when `range_number`, both ends included and either one
  # optional.
  def self.for(type_de_champ)
    range_min, range_max = NumberLimitValidator.range(type_de_champ)
    min = [type_de_champ.positive_number? ? 0 : nil, range_min].compact.max
    limits = { min:, max: range_max } if min || range_max

    new(integer: type_de_champ.integer_number?, limits:).restrict(Logic::GreaterThanEq, min).restrict(Logic::LessThanEq, range_max)
  end
