...
class Logic::Solver
  ...
  def branch_conditions(term)
    case term
    when Logic::Or
      term.operands.flat_map { [it, *branch_conditions(it)] }
    when Logic::And
  ...
  # The values a champ starts out with depend on the champ alone, while the
  # search asks for them at every step: computed once per source.
  def possible_values(source)
    @possible_values ||= {}
    return @possible_values[source] if @possible_values.key?(source)

    @possible_values[source] = source.possible_values(@type_de_champs)
  end
