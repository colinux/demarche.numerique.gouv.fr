...
class Conditions::ConditionsErrorsComponent < ApplicationComponent
  ...
  def humanize_comparison(comparison)
    "« #{operator_label(comparison).downcase} #{humanize_value(comparison)} »"
  end

  # The label the editor gives the operator (see
  # ConditionsComponent#compatibles_operators_for_select): « est » and
  # « n’est pas » for anything but a number
  def operator_label(comparison)
    case comparison
    when Logic::NotEq then t('is_not', scope: 'logic')
    when Logic::Eq
      comparison.left.type(@source_tdcs) == :number ? t(comparison.class.name, scope: 'logic.operators') : t('is', scope: 'logic')
    else t(comparison.class.name, scope: 'logic.operators')
    end
  end
