...
describe 'Dossier::Recovery::LifeCycle' do
  describe '.load_export_destroy_and_import' do
    ...
    it 'reloads the full grappe', :slow do
      ...
      expect(repetition(reloaded_dossier).rows.flat_map(&:flat_children).map(&:type)).to match_array(["Champs::PieceJustificativeChamp", "Champs::PieceJustificativeChamp", "Champs::PieceJustificativeChamp"])
