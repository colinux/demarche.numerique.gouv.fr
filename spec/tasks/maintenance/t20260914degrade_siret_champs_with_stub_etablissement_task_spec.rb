...
module Maintenance
  RSpec.describe T20260914degradeSiretChampsWithStubEtablissementTask do
    ...
    describe "#process" do
      ...
      # Etablissement validates the presence of its siret, so a blank one can
      # only come from a raw write — which is how these rows reached production.
      context "when only the legacy value column holds the siret" do
        before do
          champ.update_columns(external_id: nil, value: siret)
          etablissement.update_columns(siret: nil)
        end

        it "takes it from there rather than declaring the champ empty" do
          process
      ...
      # Only the stub makes the dossier read as degraded, and can_terminer?
      # reads every filled champ, blank ones included: leaving it would block
      # the dossier for good over a champ nobody ever filled.
      context "when no siret survives anywhere" do
        before do
          champ.update_columns(external_id: nil, value: nil)
          etablissement.update_columns(siret: nil)
        end

        it "drops the stub" do
          process
        ...
        it "sends the champ back to idle instead of feeding the cron a blank siret" do
          process
        ...
        it "unblocks the dossier" do
          expect { process }.to change { dossier.reload.any_etablissement_as_degraded_mode? }.from(true).to(false)
        end
