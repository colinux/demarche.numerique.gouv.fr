...
RSpec.describe Cron::RetryDegradedChampJob, type: :job do
  describe '.schedulable?' do
    ...
    it 'leaves its subclasses schedulable' do
      expect(Cron::RetryDegradedSiretChampJob).to be_schedulable
    end
  end

  describe 'replaying degraded champs' do
    ...
    context 'with a degraded champ' do
      ...
      it 'does not make the dossier look freshly modified to its instructeur' do
        expect { Cron::RetryDegradedSiretChampJob.perform_now }.not_to change { dossier.reload.updated_at }
      end
      ...
      context 'while INSEE is still down' do
        ...
        it { expect { Cron::RetryDegradedSiretChampJob.perform_now }.not_to change { champ.reload.external_state } }
      end
    end
    ...
    context 'while the API Entreprise pool is throttled' do
      ...
      it 'waits instead of adding to the flood' do
        expect { Cron::RetryDegradedSiretChampJob.perform_now }.not_to change { champ.reload.external_state }
      end
    end
    ...
    context 'with a degraded copy on the user buffer stream' do
      ...
      it 'leaves the champ fetched once the dossier is submitted' do
        ...
        Cron::RetryDegradedSiretChampJob.perform_now
    ...
    # The instructeur still reads a dossier its usager deleted, and still needs
    # the etablissement to decide on it.
    context 'when the dossier was hidden by its user' do
      let(:external_state) { 'degraded' }

      before { dossier.update_column(:hidden_by_user_at, 1.day.ago) }

      it 'replays it anyway, for the administration that can still read it' do
        expect { Cron::RetryDegradedSiretChampJob.perform_now }
          .to change { champ.reload.external_state }.from('degraded').to('waiting_for_fix')
      end
    end

    context 'when the dossier was hidden by the administration' do
      let(:external_state) { 'degraded' }

      before { dossier.update_column(:hidden_by_administration_at, 1.day.ago) }
