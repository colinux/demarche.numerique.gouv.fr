...
describe Logic::Solver do
  ...
  context 'with alternatives on the same champ' do
    let(:condition) { ds_and([ds_or([ds_eq(n, constant(1)), ds_eq(n, constant(2))]), ds_or([ds_eq(n, constant(3)), ds_eq(n, constant(4))])]) }

    before { allow(Logic::PossibleValues).to receive(:for).and_call_original }

    it 'computes the possible values of the champ once' do
      expect(errors).to be_present
      expect(Logic::PossibleValues).to have_received(:for).once
    end
  end
