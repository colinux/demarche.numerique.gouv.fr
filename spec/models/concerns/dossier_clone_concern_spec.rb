    ...
    describe 'champs' do
      ...
      context 'public are duplicated' do
        ...
        context 'for Champs::Repetition with rows, original_champ.repetition and rows are duped' do
          ...
          it do
            expect(cloned_champ_repetition.rows.flat_map(&:flat_children).count).to eq(4)
            expect(cloned_champ_repetition.rows.flat_map(&:flat_children).map(&:id)).not_to eq(champ_repetition.rows.flat_map(&:flat_children).map(&:id))
